...
function maybeHighlightRow(square)
{
	// minus 1 to convert to numbering starting from zero
	const number = parseInt(square.attr("id").substring("slot".length)) - 1;
	if (isNaN(number)) {
		return;
	}
	const colour = getColourClass(square);
	if (colour == "") {
		return;
	}
	...
	var foundMainDiagonal = false;
	if (number % 6 == 0) {
		foundMainDiagonal = true;
		for (var i = 0; i < 25; i += 6) {
			let diagonalC = getColourClass(getSquareByIndex(i));
			if (diagonalC != colour) {
				foundMainDiagonal = false;
				break;
			}
		}
	}

	var foundSecondaryDiagonal = false;
	if (number >= 4 && number <= 20 && number % 4 == 0) {
		foundSecondaryDiagonal = true;
		for (var i = 4; i <= 20; i += 4) {
			let diagonalC = getColourClass(getSquareByIndex(i));
			if (diagonalC != colour) {
				foundSecondaryDiagonal = false;
				break;
			}
		}
	}
	...
	for (var i = 0; i < 5; i++) {
		const rowC = getColourClass(getSquareByCoords(i, row));
		if (rowC != colour) {
			foundRow = false;
		}
		const columnC = getColourClass(getSquareByCoords(column, i));
		if (columnC != colour) {
			foundColumn = false;
		}
	...
	if (foundMainDiagonal) {
		for (var i = 0; i < 25; i += 6) {
			highlightedSquares.push(getSquareByIndex(i));
		}
	}
	if (foundSecondaryDiagonal) {
		for (var i = 4; i <= 20; i += 4) {
			highlightedSquares.push(getSquareByIndex(i));
		}
	}
	if (foundRow) {
		for (var i = 0; i < 5; i++) {
			highlightedSquares.push(getSquareByCoords(i, row));
		}
	}
	if (foundColumn) {
		for (var i = 0; i < 5; i++) {
			highlightedSquares.push(getSquareByCoords(column, i));
		}
	}
	highlight(highlightedSquares);
}
...
async function highlight(squares)
{
	if (squares.length == 0)
		return;
	async function setColours(val, t)
	{
		for (const square of squares)
		{
			square.css("color", val);
			await sleep(t);
		}
	}
	for (var r = 0; r < 255; r += 10)
	{
		await setColours("rgb(" + r + ",0,0)", 4);
	}
	for (var r = 255; r >= 0; r -= 10)
	{
		await setColours("rgb(" + r + ",0,0)", 4);
	}
	setColours("", 0);
...
function getSquareByCoords(x, y) // 0<=x<5 && 0<=y<5
{
	return getSquareByIndex(y * 5 + x);
}
function getSquareByIndex(i)
{
	return $("#slot" + (i + 1));
}
